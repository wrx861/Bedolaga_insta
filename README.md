...
```
├── main.go            # Точка входа + CLI-роутинг (56 строк)
├── config.go          # Структура Config
├── styles.go          # Цвета + стили + баннер
├── progress.go        # Прогресс-трекер + обработка сигналов
├── ui.go              # UI-компоненты (spinner, select, input, confirm, progress bar)
├── helpers.go         # Print-хелперы
├── utils.go           # Системные утилиты (runCmd, generateToken, validateDomain...)
├── system.go          # Проверки ОС + установка пакетов (Docker, Nginx, Caddy)
├── setup.go           # Интерактивная настройка (12 шагов)
├── envfile.go         # Генерация .env (200+ переменных)
├── compose.go         # Docker Compose файлы + клонирование репозитория
├── proxy.go           # Nginx + Caddy + SSL
├── docker.go          # Запуск Docker + firewall
├── management.go      # Скрипт управления `bot` + финальная информация
